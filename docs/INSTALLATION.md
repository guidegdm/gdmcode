...
binary. It includes release checksums, the applicable licenses/notices, and
installation scripts. The application source is maintained separately in a
private repository and is not required for installation.
...
bundle, then optionally downloads and verifies a public model. It installs to
...
`download_model.sh` downloads the selected submission model. The Windows
